  ...
  return (
    <div className="relative w-full">
      <input
        ...
        className={cn(
          'h-10 w-full rounded-md border border-input bg-background px-3 pr-16 text-sm outline-none ring-offset-background',
          'placeholder:text-muted-foreground focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60',
          className,
        )}
      />
      {showClearButton ? (
        <button
          ...
          className="absolute right-9 top-1/2 -translate-y-1/2 rounded-sm p-1 text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          onMouseDown={(event) => {
            event.preventDefault();
          }}
          onClick={() => {
...
export function ComboboxContent({ className, children }: ComboboxContentProps) {
  ...
  return (
    <div
      className={cn(
        'absolute top-[calc(100%+4px)] z-50 w-full overflow-hidden rounded-md border border-border bg-popover p-1 shadow-md',
        className,
      )}
    >
      {children}
